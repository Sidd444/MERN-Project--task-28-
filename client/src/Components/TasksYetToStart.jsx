...
function TaskItem({ firstArr,task, fetchTasks, updateCurrentTask, toggleEditTaskForm,setFirstArr,changeStatus }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      setFirstArr(firstArr.filter(t => t._id !== task._id)); // Remove the task from the state
    } catch (error) {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      setFirstArr(firstArr.map(t => t._id === task._id ? { ...t, status: newStatus } : t)); // Update the task's status in the state
      let obj={}
      firstArr.forEach((e,i)=>{
        if(e._id==task._id) obj=e;
      });
      changeStatus(newStatus,obj,firstArr,"first array");
    } catch (error) {
...
export default function TasksYetToStart(props) {
  ...
  props.updateCounter1(props.firstArr.length);
